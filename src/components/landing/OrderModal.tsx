...
export function OrderModal({
  isOpen, onClose, productId, productName, price, brandColor, onOrderSuccess,
}: OrderModalProps) {
  ...
  const form = useForm<OrderFormData>({
    ...
    defaultValues: { full_name: "", phone: "", quantity: 1, city: "", delivery_address: "", notes: "" },
  });
            ...
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-2.5 sm:space-y-3">
              <FormField control={form.control} name="full_name" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Votre nom" className="text-base h-10 sm:text-sm" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="phone" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="07 XX XX XX XX" type="tel" className="text-base h-10 sm:text-sm" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <div className="flex gap-2.5">
                <FormField control={form.control} name="quantity" render={({ field }) => (
                  <FormItem className="flex-1">
                    ...
                    <FormControl><Input type="number" min={1} className="text-base h-10 sm:text-sm" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="delivery_address" render={({ field }) => (
                  <FormItem className="flex-[2]">
                    ...
                    <FormControl><Input placeholder="Quartier, rue..." className="text-base h-10 sm:text-sm" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
              ...
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Instructions spéciales..." className="text-base h-10 sm:text-sm" {...field} /></FormControl>
                </FormItem>
              )} />
              ...
              <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-gray-50 border">
                ...
                <span className="text-base font-bold" style={{ color: brandColor }}>{formatPrice(total)}</span>
              </div>

              {error && <p className="text-xs text-red-600 bg-red-50 p-2.5 rounded-lg">{error}</p>}

              <Button type="submit" disabled={isSubmitting} className="w-full h-12 text-sm font-semibold text-white rounded-xl" style={{ backgroundColor: brandColor }}>
                {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <ShoppingCart className="w-5 h-5 mr-2" />}
